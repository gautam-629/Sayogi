            ...
            <div className='col-span-9 mt-28'>
                ...
                <div>
                    <table className='bg-secBackColor text-textColor w-5/6 text-center rounded-md'>
                        ...
                        <tbody>
                            {
                                hiredUser && Array.isArray(hiredUser)
                                && hiredUser.map((data) => (
                                    <tr key={data._id}>
                                        <td className='py-2'>{++count}</td>
                                        <td onClick={()=>sendUser(data.sender)}  className='py-2'>
                                            <img className=' inline-block h-12 w-12 ml-3 border-4 rounded-full
                                        cursor-pointer object-cover border-gray-400' src={`http://localhost:5000${data.sender.avatar}`} alt="profile" /> <br />
                                            <span className='cursor-pointer'>{data.sender.name}</span>
                                        </td>
                                        <td onClick={()=>sendUser(data.receiver)} className='py-2'>
                                            <img className=' inline-block h-12 w-12 ml-3 border-4 rounded-full
                                        cursor-pointer object-cover border-gray-400' src={`http://localhost:5000${data.receiver.avatar}`}   alt="profile" /> <br />
                                            <span className='cursor-pointer'>{data.receiver.name}</span>
                                        </td>
                                        <td className='py-2'>{`Rs.${data.paymentInfo.amount}`}</td>
                                        <td style={data.paymentInfo.status === 'pending' ? { color: 'red' } : { color: 'green' }}>{data.paymentInfo.status}</td>
                                        <td className='pl-5'>
                                          
                                                <img onClick={(e)=>sendDetail(data.paymentInfo)} className='w-10 inline mx-2 cursor-pointer' src="/img/detail.png" alt="" />
