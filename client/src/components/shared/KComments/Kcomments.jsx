   ...
   return (
      <div className='bg-secBackColor relative p-4 h-auto w-96 rounded-md'>
         ...
         {comments.map((com) => {
            return (
               <div key={com._id} className='flex items-center gap-4 mt-6'>
                 ...
                 <img onClick={(e)=>handleProfileClick(com.user._id)} className='inline-block h-12 w-12 ml-3 border-4 rounded-full cursor-pointer 
                  object-cover border-gray-400' src={`http://localhost:5000${com.user.avatar}`} alt="profile" />
                  
                  <div className='bg-tecBackColor inline-block rounded-md px-2'>
                     <span className='text-textColor pl-14 font-bold'>{com.user.name}</span>
         ...
         {isAuth && user?.activated && user?.serviceSeeker &&
          ...
          <div>
             <input value={content} onChange={(e)=>setContent(e.target.value)} className='bg-tecBackColor outline-none border-none text-textColor py-2 px-8 rounded-md' type="text" />
             ...
             <img onClick={handleSubmit} className='h-5 w-7 pl-2 inline-block cursor-pointer' src='img/send.png' alt="send" />
          </div>
       </div>
         }:
      </div>
