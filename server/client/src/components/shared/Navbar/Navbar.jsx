                  ...
                  notification.map((noti) => (
                    <>
                      ...
                      >
                        ...
                        <div
                          onClick={(e) => handleViewDetail(noti)}
                          className="mr-2 inline-block"
                        >
                          <span className="text-textColor">{`${noti.sender.name} Request You`}</span>
                          <h2 className="text-blue mt-3 ml-8 font-bold cursor-pointer">
                            View Detail
                          </h2>
                        </div>
                      </div>
          ...
          {isAuth && user?.activated ? (
            <div className="relative mr-10">
              ...
              {open && (
                <div
                  ref={menuRef}
                  className="bg-secBackColor p-4 w-32 shadow-lg absolute rounded-sm -left-8 top-20"
                >
                  <ul>
                    {isAuth && user?.activated && user?.role === "admin" && (
                      <Link to={"/dashboard"}>
                        <li className="text-textColor font-bold text-lg p-2 cursor-pointer rounded">
                          DashBoard
                        </li>
                      </Link>
                    )}
                    <li
                      ...
                      className="text-textColor font-bold text-lg p-2 cursor-pointer rounded"
                    >
                      Logout
                    </li>
                    <li
            ...
            <Link to={"/authenticate"}>
              <span className="text-textColor mr-20 cursor-pointer text-lg font-bold">
                login
              </span>
            </Link>
          )}
